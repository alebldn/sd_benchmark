...
/// Signature scheme for selective disclosure implemented as specified in "Efficient Redactable Signature and Application to Anonymous Credentials".
///
#[derive(Debug, Clone)]
...
impl PSSignature {
    ...
    /// Getter function to retrieve the sigma_1 attribute from the signature
    pub fn sigma_1(&self) -> G1Affine {
        self.sigma_1
    }
    /// Getter function to retrieve the sigma_2 attribute from the signature
    pub fn sigma_2(&self) -> G1Affine {
        self.sigma_2
    }
    /// Getter function to retrieve the sigma_tilde_1 attribute from the signature
    pub fn sigma_tilde_1(&self) -> G2Affine {
        self.sigma_tilde_1
    }
    /// Getter function to retrieve the sigma_tilde_2 attribute from the signature
    pub fn sigma_tilde_2(&self) -> G2Affine {
        self.sigma_tilde_2
    }


    /// Function to generate cryptographic key material.
    ///
    /// # Arguments
    /// * `messages_len` - Total length of the message array.
    /// * `rng` - Random Number Generator to create the key pair with.
    ///
    /// # Returns
    /// A result containing the PSPrivateKey, PSPublicKey pair or a string highlighting an error, if it occurs.
    ...
    /// Function to generate a signature over an array of messages.
    ///
    /// # Arguments
    /// * `sk` - Secret Key of the signer.
    /// * `messages` - Message array to be signed.
    /// * `rng` - Random Number Generator to generate the signature with.
    ///
    /// # Returns
    /// A result containing a PSSignature or a string containing an error, if it occurs.
    ...
    /// Function to adapt a signature created from a set of messages to be verified with a subset of them.
    ///
    /// # Arguments
    /// * `pk` - Public Key of the signer.
    /// * `signature` - Signature to derive from.
    /// * `messages` - Message array previously signed.
    /// * `disclosed_indices` - Array of disclosed indices.
    /// * `rng` - Random Number Generator to generate the signature with.
    ///
    /// # Returns
    /// A result containing a derived PSSignature or a string containing an error, if it occurs.
    pub fn derive(pk: &PSPublicKey, signature: &PSSignature, messages: &Vec<PSMessage>, disclosed_indices: &Vec<usize>, rng: &mut StdRng) -> Result<Self, String> {
        ...
        let undisclosed_indices: Vec<usize> = complementary_indices(disclosed_indices, messages.len());

        let sigma_tilde_1_prime: G2Affine = (signature.sigma_tilde_1() * r).into();
        let sigma_tilde_2_prime: G2Affine = ((signature.sigma_tilde_2() * r) + sigma_tilde_1_prime * t).into();
    ...
    /// # Arguments
    /// * `pk` - Public Key of the signer.
    /// * `signature` - Derived signature of the disclosed message array.
    ///
    /// # Returns
    /// A result containing a string containing an error, if it occurs.
    pub fn verify(pk: &PSPublicKey, signature: &PSSignature, disclosed_messages: &Vec<PSMessage>) -> Result<bool, String> {

        #![allow(unused_labels)]
        'first_check: {
            let mut product: G1Affine = (pk.capital_x() + signature.sigma_1()).into();
            let mut capital_y_i: G1Affine;
            ...
            if Bls12_381::pairing(product, signature.sigma_tilde_1()) != Bls12_381::pairing(pk.g(), signature.sigma_tilde_2()) {
        ...
        'second_check: {
            ...
            if Bls12_381::pairing(signature.sigma_1(), product) != Bls12_381::pairing(signature.sigma_2(), pk.g_tilde()) {
...
impl Serialize for PSSignature {
    ...
    /// Implementation of the Serialize trait for PSSignature.
